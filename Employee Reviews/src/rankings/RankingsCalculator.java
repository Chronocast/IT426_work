...
public class RankingsCalculator
{
	...
	public void printRankings(IStrategy strategy, int year)
    {
		...
		/*
		    1: Loop over each name available using the names Set<T> (names)
		    2: Retrieve reviews for that employee using the reviewData Map<K, V>  (reviewData)
		...
		for (String name: names) {
			rankedList.add(new RankedReview(name, strategy.getScore(year, reviewData.get(name))));
		}
